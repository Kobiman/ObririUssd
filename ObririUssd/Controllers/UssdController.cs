...
namespace ObririUssd.Controllers
{
    ...
    {
        static ConcurrentDictionary<string, string> _previousState;
        static ConcurrentDictionary<string, string> PreviousState = _previousState ?? new ConcurrentDictionary<string, string>();
        ...
        public async Task<IActionResult> Index([FromBody] UssdRequest request)
        {
            if (PreviousState.TryGetValue(request.MSISDN, out state))
            {
                ...
                PreviousState.TryAdd(request.MSISDN, tt+"1");
                PreviousState.TryGetValue(request.MSISDN, out state);
            }

            if (string.IsNullOrWhiteSpace(state) && request.USERDATA == "1")
            {
            ...
            if (state == "11" && request.USERDATA == "1")
            {
                PreviousState.TryAdd(request.MSISDN, request.USERDATA);
                return Ok(new UssdResponse
                {
            ...
            //Main Option 1 sub 2---------------------------------------------------------------------------------------------------------------------
            if (state == "11" && request.USERDATA == "2")
            {
                PreviousState.TryAdd(request.MSISDN, request.USERDATA);
                return Ok(new UssdResponse
                {
